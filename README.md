...
| **結果比較** | 2 本の SQL（変更前 / 変更後）を構文比較し、実行結果に影響しそうな差分をカテゴリ別に表示 |
...
- **ORDER BY（行順）も比較する** をオンにすると並び順の差も「結果差分」として扱います（デフォルトはオフ＝行の集合として比較）
- エイリアスの違いは実テーブル名に解決してから比較します
- **構文上の推定**です。意味的に等価でも構文木が違う場合は差分と出ることがあります
...
- 結果比較における意味的等価の厳密証明（構文構造の差分検出まで）
