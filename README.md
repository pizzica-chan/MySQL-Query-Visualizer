...
主軸は **構文差分の列挙**（実行前の構文レビュー）で、意味等価の証明はしません。
...
- よくあるリファクタ（相関 EXISTS ↔ JOIN+DISTINCT、INNER の WHERE↔ON など）は **要確認ヒント**（黄）として補助表示します。結果セット推定は「異なる」のままです
- 「結果セットが同じ」は構文カテゴリに差分がないことの推定であり、意味的に等価でも構文が違う場合は差分と出ます
